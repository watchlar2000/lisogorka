<script lang="ts">
	...
	import type { OnSaveParams, ProjectState } from '../../new/types';
	...
	const defaultSelectedCategory = data.project.category;
	...
	$effect(() => {
		const { title, coverImage, description, category, images } = project;
		const { id, url, alt } = coverImage;
		projectState.title = title;
		projectState.coverImage = { id, url, alt, file: null };
		projectState.description = description;
		projectState.category = category;
		projectState.images = images;
	});
...
>
	<label for="title" class="">
		...
		<input type="text" id="title" name="title" value={projectState.title} />
	</label>
